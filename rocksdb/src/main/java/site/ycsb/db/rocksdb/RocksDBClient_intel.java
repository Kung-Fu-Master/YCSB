...
public class RocksDBClient extends DB {
  ...
  private static final String BACKGROUD_COMPACT_FLUSH_THREAD = "BG_THREAD";
  private static final String COLUMN_FAMILY_NAMES_FILENAME = "CF_NAMES";
  private static final String COLUMNFAMILY_NUM = "CF_NUM";
  private static final String LEVELNUM = "LEVEL_NUM";
  private static final String READONLY = "READ_ONLY";
  ...
  public void init() throws DBException {
    synchronized(RocksDBClient.class) {
      cfNum = Integer.parseInt(getProperties().getProperty(COLUMNFAMILY_NUM, "4"));
      if(rocksDb == null) {
  ...
  private RocksDB initRocksDB() throws IOException, RocksDBException {
    ...
    final int rocksThreads = Runtime.getRuntime().availableProcessors() * 2;  
    final int bgThreads = Integer.parseInt(getProperties().getProperty(BACKGROUD_COMPACT_FLUSH_THREAD, "2"));  
    final int levelNum = Integer.parseInt(getProperties().getProperty(LEVELNUM, "5"));  
    String readonly = getProperties().getProperty(READONLY, "True");  
    final List<String> cfNames = loadColumnFamilyNames();  
    final List<ColumnFamilyOptions> cfOptionss = new ArrayList<>();  
    final List<ColumnFamilyDescriptor> cfDescriptors = new ArrayList<>();  
    LOGGER.info("RocksDB cfNames: " + cfNames.size());  
    if (cfNames.isEmpty()) {  
      cfNames.add("default");
      for (int i = 0; i < cfNum; ++i) {  
        cfNames.add(Integer.toString(i));
      }
    }
    
    for(final String cfName : cfNames) {
      LOGGER.info(cfName + " loading");
      MemTableConfig memTableConfig = new HashLinkedListMemTableConfig()
          .setBucketCount(1<<18)  
          .setHugePageTlbSize(0)  
          .setBucketEntriesLoggingThreshold(4096)  
          .setIfLogBucketDistWhenFlush(true)  
          .setThresholdUseSkiplist(32);  
      
      TableFormatConfig tableFormatConfig = new PlainTableConfig()  
          .setStoreIndexInFile(true)  
          .setIndexSparseness(1)  
          .setHashTableRatio(0.8)  
          .setBloomBitsPerKey(0);  
      
      final ColumnFamilyOptions cfOptions = new ColumnFamilyOptions()  
          .optimizeLevelStyleCompaction()  
          .setMinWriteBufferNumberToMerge(1)  
          .setNumLevels(levelNum)  
          .setMaxWriteBufferNumber(160)  
          ...
          .setMemtablePrefixBloomSizeRatio(0)  
          .setWriteBufferSize(128<<20)  
          .setTargetFileSizeBase(32<<20)  
          .setMaxBytesForLevelMultiplier(4)  
          .setMemTableConfig(memTableConfig)  
          .setTableFormatConfig(tableFormatConfig)  
          .setLevelCompactionDynamicLevelBytes(true)  
          .useFixedLengthPrefixExtractor(4);  
    ...
    if(cfDescriptors.isEmpty()) {  
      ...
      LOGGER.info("load old");  
      Statistics st = new Statistics();  
      st.setStatsLevel(StatsLevel.EXCEPT_TIME_FOR_MUTEX);  
      final DBOptions options = new DBOptions()  
          .setStatistics(st)  
          ...
          .setMaxLogFileSize(32<<20)  
          .setKeepLogFileNum(1)  
          .setMaxManifestFileSize(32<<20)  
          .setWalDir("/dev/shm/ycsb_wal")  
          .setWalTtlSeconds(1800)  
          .setMaxBackgroundFlushes(bgThreads)  
          .setMaxBackgroundCompactions(bgThreads)  
          .setUseAdaptiveMutex(true)  
          .setDeleteObsoleteFilesPeriodMicros(600*1000000)  
      ...
      final RocksDB db = readonly.equals("True")?  
          (RocksDB.openReadOnly(options, rocksDbDir.toAbsolutePath().toString(), cfDescriptors, cfHandles)):  
          (RocksDB.open(options, rocksDbDir.toAbsolutePath().toString(), cfDescriptors, cfHandles));  
      for(int i = 0; i < cfNum; i++) {  
        LOGGER.info("add " + cfHandles.get(i+1).getDescriptor().toString());  
        //COLUMN_FAMILIES.put(i, new ColumnFamily(cfHandles.get(i+1), cfOptionss.get(i)));  
        COLUMN_FAMILIES.put(Integer.toString(i), new ColumnFamily(cfHandles.get(i+1), cfOptionss.get(i)));  
      }  
